...
async function exportSmallBanner({title,subtitle,subtitleMode,discountValue,discountLang,showSubtitle,productImages,bgColor,bgImage,bgMode,bannerStyle,titleColor,subtitleColor}){
  ...
  ctx.restore();
  ctx.save();ctx.fillStyle=titleColor||"#fff";ctx.font=`800 15px 'Barlow Condensed','Arial Narrow',sans-serif`;ctx.textBaseline="middle";
  ...
  if(showSubtitle){ctx.fillStyle=subtitleColor||"#fff";if(subtitleMode==="discount"){ctx.font=`700 10px 'Barlow Condensed',sans-serif`;ctx.globalAlpha=.9;ctx.fillText(discountLang==="id"?"Diskon s.d.":"Discount up to",16,ty+2,W*.55);ctx.globalAlpha=1;ctx.font=`800 14px 'Barlow Condensed',sans-serif`;ctx.fillText(discountValue+"%",16,ty+14,W*.55);}else{ctx.font=`700 10px 'Barlow Condensed',sans-serif`;ctx.globalAlpha=.88;ctx.fillText(subtitle,16,ty+2,W*.55);ctx.globalAlpha=1;}}
  ctx.restore();
...
async function exportBigBanner({title,titleColor,subtitle,subtitleColor,showSubtitle,logo,logoScale,logoX,logoY,heroImages,bgColor,bgImage,bgMode,textAlign}){
  ...
  ctx.restore();
  ctx.save();ctx.fillStyle=titleColor||"#fff";ctx.font=`800 13.5px 'Barlow Condensed',sans-serif`;ctx.textBaseline="alphabetic";
  ...
  if(showSubtitle&&subtitle){ctx.fillStyle=subtitleColor||"rgba(255,255,255,.88)";ctx.font=`600 12px 'Barlow Condensed',sans-serif`;ctx.fillText(subtitle,tx,ty2+4,W*.58);}
  ctx.restore();
...
async function exportCustomBanner({canvasW,canvasH,bgMode,bgColor,bgColor2,bgImage,cornerRadius,elements}){
  ...
  for(const el of elements){
    ...
    else if(el.type==="text"){const fs=el.fontSize||24;ctx.font=`${el.fontWeight||700} ${fs}px ${el.fontFamily||"sans-serif"}`;ctx.fillStyle=el.color||"#111";ctx.textAlign=el.align||"left";ctx.textBaseline="top";(el.text||"Text").split("\n").forEach((l,i)=>ctx.fillText(l,0,i*fs*1.2));}
    ...
    <div style={{width:SBW*SB_SCALE,height:(SBH+OVERFLOW)*SB_SCALE,position:"relative",overflow:"hidden",flexShrink:0}}>
      <div style={{position:"absolute",bottom:0,left:0,width:SBW*SB_SCALE,height:SBH*SB_SCALE,borderRadius:SBR*SB_SCALE,overflow:"hidden",background:gradient,boxShadow:"0 4px 20px rgba(0,0,0,.18)"}}>
        ...
        <div style={{position:"absolute",top:"50%",transform:"translateY(-50%)",left:16*SB_SCALE,right:0,zIndex:4}}>
          <div style={{fontFamily:"'Barlow Condensed','Arial Narrow',sans-serif",fontWeight:800,fontSize:15*SB_SCALE,lineHeight:1.05,color:titleColor||"#fff",whiteSpace:"pre-wrap",wordBreak:"break-word"}}>{title||"Judul banner kamu"}</div>
          {showSubtitle&&<div style={{marginTop:2*SB_SCALE}}>{subtitleMode==="discount"?<><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:10*SB_SCALE,lineHeight:1,color:subtitleColor||"#fff",opacity:.9}}>{discountLang==="id"?"Diskon s.d.":"Discount up to"}</div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:14*SB_SCALE,lineHeight:1,color:subtitleColor||"#fff"}}>{discountValue}%</div></>:<div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:10*SB_SCALE,lineHeight:1,color:subtitleColor||"#fff",opacity:.88}}>{subtitle}</div>}</div>}
        </div>
      </div>
  ...
  return(
    <div style={{width:BBW*BB_SCALE,height:BBH*BB_SCALE,position:"relative",borderRadius:BBR*BB_SCALE,overflow:"hidden",background:gradient,boxShadow:"0 6px 32px rgba(0,0,0,.22)",flexShrink:0}}>
      ...
      <div style={{position:"absolute",bottom:20*BB_SCALE,zIndex:5,...textPos}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:13.5*BB_SCALE,lineHeight:1.15,color:titleColor||"#fff",whiteSpace:"pre-wrap",wordBreak:"break-word",textShadow:"0 2px 6px rgba(0,0,0,.25)"}}>{title||"Judul big banner kamu"}</div>
        {showSubtitle&&subtitle&&<div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:600,fontSize:12*BB_SCALE,lineHeight:1.2,color:subtitleColor||"rgba(255,255,255,.88)",marginTop:5*BB_SCALE,textShadow:"0 1px 4px rgba(0,0,0,.2)"}}>{subtitle}</div>}
      </div>
    </div>
  );
}
...
function SmallBannerEditor({onBack}){
  ...
  return(
    <div style={{height:"100vh",display:"flex",flexDirection:"column",fontFamily:"'Inter','Segoe UI',sans-serif",background:"#f5f5f5"}}>
...
function CustomBannerEditor({onBack}){
  ...
  const removeEl=id=>{setElements(p=>p.filter(e=>e.id!==id));if(selectedId===id)setSelectedId(null);};
  ...
  const addText=()=>{
    const id=newId();
    setElements(p=>[...p,{id,type:"text",text:"Your text here",x:canvasW/2,y:canvasH/2,scale:1,rotation:0,opacity:1,fontSize:36,fontWeight:700,fontFamily:"sans-serif",color:"#111111",align:"center",dispW:200,dispH:44}]);
  ...
  const startScale=useCallback((e,id,corner)=>{
    e.preventDefault();
    const el=elements.find(x=>x.id===id);if(!el)return;
    op.current={type:"scale",id,data:{mx:e.clientX,my:e.clientY,scale:el.scale,corner}};
  },[elements]);
  ...
  useEffect(()=>{
    const onMM=e=>{
      ...
      if(o.type==="drag"){
        ...
        const dx=(e.clientX-o.data.mx)/DSCALE;
        const newScale=Math.max(0.02,o.data.scale+dx/100);
        setElements(p=>p.map(el=>{
          if(el.id!==o.id)return el;
          return{...el,scale:+newScale.toFixed(3),dispW:(el.natW||el.dispW||120)*newScale,dispH:(el.natH||el.dispH||36)*newScale};
        }));
  ...
  /* Selected element sidebar panel */
  const ElPanel=()=>{
    if(!selected)return(
        ...
        {selected.type==="text"&&<>
          ...
          <div style={{marginBottom:10}}>
            ...
            <select value={selected.fontFamily||"sans-serif"} onChange={e=>updateEl(selected.id,{fontFamily:e.target.value})}
              style={{width:"100%",padding:"7px 8px",border:"1px solid #e0e0e0",borderRadius:8,fontSize:12,outline:"none",background:"#fff",color:"#111"}}>
              {["sans-serif","serif","monospace","'Barlow Condensed','Arial Narrow',sans-serif","'Georgia',serif","'Courier New',monospace"].map(f=><option key={f} value={f}>{f.split(",")[0].replace(/'/g,"")}</option>)}
            </select>
          </div>
          <div style={{marginBottom:10}}>
  ...
  return(
    <div style={{height:"100vh",display:"flex",flexDirection:"column",fontFamily:"'Inter','Segoe UI',sans-serif",background:"#f0f0f0"}}>
              ...
              return(
                <div key={el.id}
                  ...
                  {el.type==="image"?(
                    ...
                    <div style={{transform:"translate(-50%,-50%)",fontFamily:el.fontFamily||"sans-serif",fontWeight:el.fontWeight||700,fontSize:(el.fontSize||24)*DSCALE,color:el.color||"#111",whiteSpace:"pre-wrap",textAlign:el.align||"left",opacity:el.opacity??1,lineHeight:1.2,pointerEvents:"none",minWidth:40}}>
                      {el.text||"Text"}
                    </div>
                  )}
                </div>
...
export default function App(){
  ...
  if(screen==="small")  return <SmallBannerEditor  onBack={()=>setScreen("select")}/>;
  if(screen==="big")    return <BigBannerEditor    onBack={()=>setScreen("select")}/>;
  if(screen==="custom") return <CustomBannerEditor onBack={()=>setScreen("select")}/>;
  return <BannerSelect onSelect={setScreen}/>;
}
